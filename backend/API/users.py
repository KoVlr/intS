...
@users_router.get("/me", response_model=schemes.users.User)
def get_user_info(user = Depends(get_authenticated_user)):
    return user


@users_router.post("/authors", response_model=schemes.users.Author)
def become_author(db: Session = Depends(get_db), user = Depends(get_authenticated_user)):
    ...
    return crud.create_author(db, schemes.users.AuthorCreate(user_id=user.id))